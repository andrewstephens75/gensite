...
class GenSiteTemplate:
    ...
    def split_header_contents(self, file):
    ...
    def make_filename_safe_title(self, s):
    ...
    def process_source_file(self, sourceFileDef, destDir):
        ...
        source = io.StringIO(sourceFileDef.contentsFromUtf8())
        header, contents = self.split_header_contents(source)
        title = header["title"]
        author = header["author"]
        template_type = header["template_type"]

        outputFileDef = FileDef(os.path.join(destDir, self.make_filename_safe_title(title) + ".html"))
        ...
        article_text = markdown.markdown(contents)
